...
{
	if(mode)
		GPIO_PinOutSet(csn_port, csn_pin);
	else
		GPIO_PinOutClear(csn_port, csn_pin);
}
...
{
	if(level)
		GPIO_PinOutSet(ce_port, ce_pin);
	else
		GPIO_PinOutClear(ce_port, ce_pin);
}
...
uint8_t RF24::write_payload(const void* buf, uint8_t data_len, const uint8_t writeType)
{
  ...
  IF_SERIAL_DEBUG( print("[Writing %u bytes %u blanks]\n",data_len,blank_len); );
...
uint8_t RF24::read_payload(void* buf, uint8_t data_len)
{
  ...
  IF_SERIAL_DEBUG( print("[Reading %u bytes %u blanks]\n",data_len,blank_len); );
...
RF24::RF24(GPIO_Port_TypeDef _ceport, uint16_t _cepin, GPIO_Port_TypeDef _csnport, uint16_t _cspin):
  ce_pin(_cepin), csn_pin(_cspin), csn_port(_csnport), ce_port(_ceport),
  payload_size(32), addr_width(5)
{
...
bool RF24::isPowerUpReady(void)
{
	return (millis() - powerUpMillis) > 3;
}
...
void RF24::enableDynamicPayloads(void)
{
  ...
  IF_SERIAL_DEBUG(print("FEATURE=%i\r\n",read_register(FEATURE)));
