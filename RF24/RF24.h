...
private:
  ...
  uint32_t powerUpMillis;/** This is used for not waiting for power up, so other useful things can be done in the meantime.**/
...
public:
   ...
   */
  RF24(GPIO_Port_TypeDef _ceport, uint16_t _cepin, GPIO_Port_TypeDef _csnport, uint16_t _cspin);
  ...
  */
  ...
  RF24(GPIO_Port_TypeDef _ceport, uint16_t _cepin, GPIO_Port_TypeDef _csnport, uint16_t _cspin, uint32_t spispeed );
  //#endif
...
s   *
   ...
   */
  void powerUp(void) ;
  void powerUpNoWait(void);
  bool isPowerUpReady(void);
  /**
